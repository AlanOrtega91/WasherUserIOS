...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func connectToFcm() {
        FIRMessaging.messaging().connect { (error) in
            if (error != nil) {
                print("Unable to connect with FCM in app delegate. \(String(describing: error))")
            } else {
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        ...
        let url = self.applicationDocumentsDirectory.appendingPathComponent("WasherV4.sqlite")
